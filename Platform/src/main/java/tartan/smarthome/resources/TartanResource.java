...
public class TartanResource {
     ...
     */
    public TartanResource(List<TartanHouseData> houses, HomeDAO homeDAO, Integer historyTimer) {

        this.services = new ArrayList<>(houses.size());
        for (TartanHouseData house : houses) {
            TartanHomeService service = new TartanHomeService(homeDAO);
            service.initializeSettings(house, historyTimer);
            ...
            if (!service.isConnected()) {
                try {
                    ...
                    LOGGER.info("Connected to house " + service.getName() + " @ " + service.getAddress() + " : "+service.getPort());

                } catch (TartanHomeConnectException thce) {
                    LOGGER.error("Could not connect to house " + service.getName() + " @ " + service.getAddress() + " : "+service.getPort());
                }
    ...
    private TartanHomeService getHomeService(String houseName) {
        ...
        LOGGER.error("Did not get the corresponding service for house"+houseName);
        return null;
    }
    ...
    public SmartHomeView state(@PathParam("house") String house,  @Auth TartanUser user) {
        ...
        for(TartanHouseData houseObj: user.getHouses()) {
            if (houseObj.getHomeName().equals(house)) {
                LOGGER.info("Received a house GET for house: " + house);
                TartanHomeService service = getHomeService(house);
                if (service == null) return null;

                return new SmartHomeView(service.getState(), user);
            }
        }
        return null;
    ...
    @Timed
    @UnitOfWork
    public Report1View report1() {
        // There are better ways to check authorization, but this works fine
        return new Report1View();
    }
    ...
    @Timed
    @UnitOfWork
    public Report2View report2() {
        // There are better ways to check authorization, but this works fine
        return new Report2View();
    }
}
